...
public class AllocationAdapter implements AllocationPort {
    ...
    public List<Allocation> findAll() {
        return allocationRepository.findAll().stream().map(AllocationMapper::toDomain).collect(Collectors.toList());
    }
    ...
    public List<Allocation> findByUserId(String userId) {
        return allocationRepository.findByUserId(userId).stream().map(AllocationMapper::toDomain).collect(Collectors.toList());
    }

    @Override
    public List<Allocation> findByResourceId(String resourceId) {
        return allocationRepository.findByResourceId(resourceId).stream().map(AllocationMapper::toDomain).collect(Collectors.toList());
    }
}
